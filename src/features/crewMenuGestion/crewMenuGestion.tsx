	...
	const PlayerList = ({ player, index }: { player: IPlayer; index: number }) => {
		return (
			<div key={player.license} className="playerLine">
				<p className="player">
					{player.fname.toUpperCase()} {player.lname.toUpperCase()} - RANG : {player.rank}
				</p>
				<div id="buttonList">
					<button className="infoPlayer" onClick={() => handleSubmitMember("infoPlayer", player, index)}>
						<img src="https://cdn.visionrp.fr/vision-cdn-v1/crew/infoPlayer.webp" />
					</button>
					{player.rank != data.nbrRank ? (
						<button className="upPlayer" onClick={() => handleSubmitMember("upPlayer", player, index)}>
							<img src="https://cdn.visionrp.fr/vision-cdn-v1/crew/upPlayer.webp" />
						</button>
					) : (
					...
					{player.rank != 1 ? (
						<button className="downPlayer" onClick={() => handleSubmitMember("downPlayer", player, index)}>
							<img src="https://cdn.visionrp.fr/vision-cdn-v1/crew/downPlayer.webp" />
						</button>
					) : (
					...
					<button className="kickPlayer" onClick={() => handleSubmitMember("kickPlayer", player, index)}>
						<img src="https://cdn.visionrp.fr/vision-cdn-v1/crew/kickPlayer.webp" />
					</button>
				</div>
			</div>
		);
			...
			<body>
				<div style={{ width: windowSize.innerWidth, height: windowSize.innerHeight, display: "flex" }}>
					...
					<div style={{ position: "absolute", top: 265, left: 543, display: "flex", flexDirection: "column" }}>
						<img src="https://cdn.visionrp.fr/vision-cdn-v1/crew/LogoMan.webp" style={{ color: "white" }} />
					</div>
					<div style={{ position: "absolute", top: 253, left: 519, display: "flex", flexDirection: "column" }}>
						<img src="https://cdn.visionrp.fr/vision-cdn-v1/crew/LogoAround.webp" style={{ height: 46, width: 65 }} />
					</div>
					...
					{value == "Membres" && (
						<React.Fragment>
							<Input
								placeholder="Nom prénom ou rang"
							...
							<div id="listeBox">
								{playersData
									.filter(
										v =>
											(v.fname + " " + v.lname).toLocaleLowerCase().includes(search.toLocaleLowerCase()) ||
											v.rank == Number(search),
									)
									.map((player, index) => (
