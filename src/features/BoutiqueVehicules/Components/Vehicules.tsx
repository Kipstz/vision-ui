...
export const Vehicules: React.FC<State> = ({ onTypeChange }) => {
	useExitKeys();
	useMouseWheel();
	...
	const getBoutiqueData = () => {
		setStoreState("loading");
		axios
			.get("https://api-sex.sacul.cloud/data/vehicules", {
				headers: {
			...
			.then(res => {
				...
				const loreCategories = uniqueCategories.filter((item: string) => item.includes("lore"));
				...
				const otherCategories = uniqueCategories.filter(
					(item: string) => !item.includes("dlc") && !item.includes("addon") && !item.includes("lore"),
				);
				const sortedCategories = [...loreCategories, ...dlcCategories, ...addonCategories, ...otherCategories];
	...
	const buyConfirm = () => {
		...
		try {
			axios.post("https://api-sex.sacul.cloud/buy/vehicule", {
				server: "FA",
	...
	useEffect(() => {
		...
		setFilteredCatalogue(catalogue.filter(item => item.category === selectedVehiculeCategory));
		setSelectedVehicule(catalogue.filter(item => item.category === selectedVehiculeCategory)[0]);
		setSelectedVehiculePrice(
			catalogue.filter(item => item.category === selectedVehiculeCategory)[0].reduction ||
				catalogue.filter(item => item.category === selectedVehiculeCategory)[0].price,
		);
	}, [selectedVehiculeCategory]);
	...
	useEffect(() => {
		postAsync("boutiqueVehSelect", selectedVehicule);
	}, [selectedVehicule]);
	...
	return (
		<>
						...
						<div className="header">
							<div className="Title">
								...
								<div className="Price">
									...
									<img src="https://cdn.visionrp.fr/vision-cdn-v1/icons/logo.webp" />
								</div>
							</div>
						</div>
				...
				<div className="BoutiqueVehicules">
						...
						<div className="TopButton">
							...
							<span
								...
								<p style={{ background: "linear-gradient(180deg, #fae293 0%, #ffad00 100%)" }}>
									...
									<img src="https://cdn.visionrp.fr/vision-cdn-v1/icons/logo.webp" alt="logo" />
								</p>
								V Coins
							</span>
							<span
							...
							<div className="Left">
								<div className="Colors">
									{colorSelectMode === "predefined" ? (
										...
										<div className="CustomColors">
											<div className="title">
												<img
													src={"https://cdn.visionrp.fr/vision-cdn-v1/BoutiqueVehicules/arrow_left.webp"}
													onClick={() => {
														playOnHoverSound();
								...
								<div className="Performance">
									...
									<div>
										<Input
											...
											onChange={e => {
												updateSelectedPerformance(parseInt(e.target.value));
											}}
											min={1}
											max={5}
											style={{
												background: getLinearGradientPerPerformance(selectedPerformance),
											}}
											type="range"
										/>
										<output>{selectedPerformance}</output>
									</div>
								</div>
								...
								<div className="TypeChange">
									<div className="buttons">
										...
										<div className="button" onClick={() => onTypeChange("air")}>
											<img src={AirIcon} />
										</div>
									</div>
									<div className="title">
										...
										{vehiculeCategories.map((category, index) => (
											<div
												className={`Category ${category === selectedVehiculeCategory ? "active" : ""} ${
													category === "lore" ? "exclu" : newCarInCategory(category) ? "new" : ""
												}`}
												key={index}
												onClick={() => {
													setSearchValue("");
												...
												<img src={`https://cdn.visionrp.fr/vision-cdn-v1/BoutiqueVehicules/${category}.webp`} />
												<h4>{category}</h4>
											</div>
										))}
									</div>
					...
					{catalogue.length > 0 && (
						<div className="Footer">
							<img
								src="https://cdn.visionrp.fr/vision-cdn-v1/concessvoiture/fast-forward.webp"
								className="LeftArrow"
								onClick={() => {
									prev();
									playOnHoverSound();
			...
			<ButtonHints
				buttons={{
					["https://cdn.visionrp.fr/vision-cdn-v1/BoutiqueVehicules/mouse.webp"]: "Zoomer/Dézoomer",
					Q: "Tourner à gauche",
					D: "Tourner à droite",
