...
export const Nautic: React.FC<State> = ({ onTypeChange }) => {
	useExitKeys();
	useMouseWheel();
	...
	const getBoutiqueData = async () => {
		...
		await axios
			.get("https://api-sex.sacul.cloud/data/nautics", {
				headers: {
	...
	const buyConfirm = () => {
		...
		try {
			axios.post("https://api-sex.sacul.cloud/buy/nautic", {
				server: "FA",
				color: color,
	...
	useEffect(() => {
		...
		postAsync("boutiqueVehSelect", { ...selectedVehicule, category: "bateaux" });
	}, [selectedVehicule]);
									...
									<div className="list">
										{vehiculeCategories.map((category, index) => (
											<div
												...
												<img
													src={`https://cdn.visionrp.fr/vision-cdn-v1/BoutiqueVehicules/vision/${category}.webp`}
												/>
												<h4>{category}</h4>
											</div>
										))}
